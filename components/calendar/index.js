...
Component({
  ...
  methods: {
    ...
    yunXian: function () {
      ...
      // 默认选择当天和明天日期、超过12点则明天、后天
      let startDate = {
        getTime: myDate.getTime(),
        text: "入住",
        xuanDayShi: day,
        xuanShiJian: `${year}-${month}`
      };
      let endDate = {
        chaDay: 1,
        getTime: myDate.getTime() + 1000*60*60*24,
        text: "离开",
        xuanDayShi: day + 1,
        xuanShiJian: `${year}-${month}`
      };
      const toDay = new Date().getDate();
      if (new Date().getHours() >= 12 && day == toDay) {
        startDate = {
          ...startDate,
          xuanDayShi: day + 1,
          getTime: myDate.getTime() + 1000*60*60*24,
        ...
        endDate = {
          ...endDate,
          xuanDayShi: day + 2,
          getTime: myDate.getTime() + (1000*60*60*24) * 2,
        };
      }

      // 默认值、点击开始限制一天问题
      // xuanZheData = [startDate, endDate];
      this.setData({
    ...
    dianXuan: function (e) {
      ...
      that.data.xuanri = xuanYue + '-' + xuanRi;
      var xsw_Data=[]
